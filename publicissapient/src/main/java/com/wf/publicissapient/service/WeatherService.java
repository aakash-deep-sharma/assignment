package com.wf.publicissapient.service;
...
import com.wf.publicissapient.client.OpenWeatherClient;
import com.wf.publicissapient.dto.WeatherForecastDto;
import com.wf.publicissapient.dto.WeatherResponseDto;
...
import com.wf.publicissapient.rule.WeatherRule;
...
public class WeatherService {
	...
	public WeatherResponseDto get3DayForecast(String city, boolean offlineMode) {
		...
		return processPayload(city, payload, "Live Production Data Stream");
	}
	...
	public WeatherResponseDto fallbackWeatherData(String city, boolean offlineMode, Throwable t) {
		...
		if (t instanceof feign.FeignException.NotFound || t.getCause() instanceof feign.FeignException.NotFound) {
			...
			errorResponse.setNotes("City '" + city + "' not found in OpenWeather database.");
		...
		if (cache != null) {
			...
			if (cachedData != null) {
				...
				log.info("Circuit Breaker / Retry triggered fallback for city: {}. Using cached data", city);
				// Update the user notes so they know they are viewing a cached snapshot
				cachedData.setNotes("Offline Mode / Fallback Active: Serving last successfully cached data.");
		...
		// city query
		log.info("Circuit Breaker / Retry triggered fallback for city: {}. No cached data", city);

		return new WeatherResponseDto(city, "Weather service is down.", List.of(), "NO_DATA");
	}
